...
const int iden_type_of_access = 11;
...
int reader_writer(int writer_count, std::vector<int> data) {
    ...
    if (writer_count < proc_size - 2 && writer_count > 0) {
            ...
            while (true) {
                ...
                if (!ready_recv ) {
                    ...
                    if (flagg && access == access_W) {
                        ...
                        MPI_Recv(&data[0], datasize, MPI_INT, MPI_ANY_SOURCE, statusMpi.MPI_SOURCE, MPI_COMM_WORLD, &statusMpi);
                    ...
                    if (flagg && access == access_R) {
                        ...
                        std::cout<<std::endl;
                        std::cout << "Current readers count: " << readercount << std::endl;
                        data.resize(datasize);
                        MPI_Isend(&data[0], datasize, MPI_INT, statusMpi.MPI_SOURCE, access_R, MPI_COMM_WORLD, &requestMpi);
                        ++transition;
                        ready_recv = 1;
        ...
        if (proc_rank > 0 && proc_rank < writer_count) { //writer
            ...
            int i = data.size();
            MPI_Send(&changedata, 1, MPI_INT, 0, 66, MPI_COMM_WORLD); // if modified datasize, then send size of recv buffer
            if (changedata) {
    ...
    std::cout<<"______datasize________"<<datasize<<std::endl;
    return datasize;
}